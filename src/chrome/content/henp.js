...
function henp_update() {
	...
	var henpEnabled = preferencesService.getBoolPref("henp.enabled"),
		targetTime = preferencesService.getCharPref("henp.targetTime"),
		toleranceLimit = preferencesService.getCharPref("henp.limitTime"),
		henpPanel = document.getElementById('henp_panel');
	...
	if ((timeLeft.getDate() === now.getDate()) && (hoursLeft >= 1 || minutesLeft >= 1)) {
		...
		if (minutesLeft > 1 || (hoursLeft > 0 && minutesLeft > 0)) { 
			timeLeftString += ((minutesLeft >= 10) ? minutesLeft : ('0' + minutesLeft)) + 'm ';
		}
...
function henp_play_file() {
	...
	var player = Components.classes["@mozilla.org/sound;1"].getService(Components.interfaces.nsISound),
		...
		fileName = preferencesService.getCharPref("henp.alertSoundFile"),
